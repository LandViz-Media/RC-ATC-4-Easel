...
// Uses the proven v0.1.6 parser and renderer without changing their geometry logic.
...
function showPopover(e,op){
  const b=op.parsed.bounds;
  pop.innerHTML=
    `<strong>${escapeHTML(op.file.name)}</strong><br>`+
    `${op.tool?toolName(op.tool):"Tool not assigned"}<br>`+
...
function renderCombined(){
  ...
  combinedMeta.textContent=operations.length
    ...
    : "Add operations above to see the combined preview.";

  drawCombined();
}

// Combined preview uses the same move interpretation and arc math as the
// proven standalone renderer; only its canvas coordinate transform is different.
function drawCombined(){
  ...
  const w=canvas.width=900,h=560;
  ...
  if(!active.length){
    ...
    ctx.fillText("Assign a tool and select at least one visible tool.",w/2,h/2);
    return;
  }

  const bounds=active.reduce((b,o)=>({
    minX:Math.min(b.minX,o.parsed.bounds.minX),
    minY:Math.min(b.minY,o.parsed.bounds.minY),
    maxX:Math.max(b.maxX,o.parsed.bounds.maxX),
    maxY:Math.max(b.maxY,o.parsed.bounds.maxY)
  }),{minX:Infinity,minY:Infinity,maxX:-Infinity,maxY:-Infinity});

  const pad=32;
  const bw=Math.max(bounds.maxX-bounds.minX,.001);
  const bh=Math.max(bounds.maxY-bounds.minY,.001);
  const scale=Math.min((w-2*pad)/bw,(h-2*pad)/bh);
  const ox=(w-bw*scale)/2;
  const oy=(h-bh*scale)/2;
  const map=p=>({
    x:ox+(p.x-bounds.minX)*scale,
    y:h-(oy+(p.y-bounds.minY)*scale)
  });

  function drawLine(a,b,dash=false){
    ctx.save();
    ctx.strokeStyle=dash?"#888888":"#222";
    ctx.lineWidth=2;
  ...
  function drawArc(m){
    ...
    if(!isFinite(r)||r<1e-8){
      drawLine(map({x:m.x1,y:m.y1}),map({x:m.x2,y:m.y2}));
      return;
    }
    ...
    for(let k=0;k<=steps;k++){
      ...
      const q=map({x:cx+r*Math.cos(a),y:cy+r*Math.sin(a)});
      if(!k)ctx.moveTo(q.x,q.y);else ctx.lineTo(q.x,q.y);
    }
    ctx.stroke();
    ctx.restore();
  }
  ...
  active.forEach(o=>o.parsed.moves.forEach(m=>{
    if(m.type==="rapid"){
      drawLine(map({x:m.x1,y:m.y1}),map({x:m.x2,y:m.y2}),true);
    }else if(m.type==="cut"){
      drawLine(map({x:m.x1,y:m.y1}),map({x:m.x2,y:m.y2}));
    }else{
      drawArc(m);
...
document.querySelector("#allTools").onclick=()=>{
  filters.querySelectorAll("input").forEach(x=>x.checked=true);
  drawCombined();
};
