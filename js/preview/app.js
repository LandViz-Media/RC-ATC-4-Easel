...
function updateProjectStatus(){
  ...
  projectStatus.textContent = unassigned
    ? `${operations.length} operation${operations.length===1?"":"s"} · ${assigned} tool${assigned===1?"":"s"} assigned · ${unassigned} unassigned`
    : `${operations.length} operation${operations.length===1?"":"s"} · ${assigned} tool${assigned===1?"":"s"} assigned`;
}
...
function renderUI(){
  ...
  operations.forEach((op,i)=>{
    ...
    if(!Number.isInteger(op.tool)){
      const warning=document.createElement("div");
      warning.className="tool-warning";
      warning.textContent="⚠ Tool not assigned — excluded from combined preview";
      info.append(warning);
    }
