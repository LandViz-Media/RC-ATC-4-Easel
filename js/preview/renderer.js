...
// The cutting geometry determines the viewport scale. Rapid moves are drawn using
// that same coordinate transform but must not force the cut geometry to shrink.
// This matches the useful behavior of the original standalone v0.1.6 preview.
...
function render(canvas,parsed,opts={}){
  ...
  if(!parsed || !parsed.bounds){
    ...
    ctx.fillText("No XY toolpath detected",w/2,h/2);
    return;
  }

  // Fit the viewport to cutting geometry, not the potentially distant rapid
  // moves used to travel between cutting locations.
  const bounds=cuttingBounds(parsed);
  ...
  if(opts.showAxes){
    ...
    const o=map({x:bounds.minX,y:bounds.minY});
...
function renderCombined(canvas,entries,visibleTools,opts={}){
  ...
  const active=entries.filter(e=>
    e.parsed &&
    e.parsed.bounds &&
    (!visibleTools.size || visibleTools.has(Number(e.tool)))
  );
  ...
  const bounds=combinedCuttingBounds(active);
